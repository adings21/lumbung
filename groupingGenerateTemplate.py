...
def generate_invoice_number(row, invoice_month_year):
    ...
    invoice_number = f"{warehouse_name}_{customer_number}_{invoice_month_year}"
    return invoice_number

def generate_journal_template(input_file, invoice_date, due_date, invoice_month_year):
    # Read input file
    ...
    jurnal_template = pd.DataFrame({
        ...
        'TrackingNo': '',
        'CustomerRefNo': '',
        '*InvoiceNumber': grouped.apply(lambda row: generate_invoice_number(row, invoice_month_year), axis=1),  # Generate Invoice Number
        'Message': '',
        'Memo': '',
...
def main():
    st.title('Generate Journal Template')
    ...
    if uploaded_file:
        ...
        if st.button('Generate Template'):
            ...
            st.download_button(
                ...
                file_name='journal_template_Fix_LinkAja.xlsx',
